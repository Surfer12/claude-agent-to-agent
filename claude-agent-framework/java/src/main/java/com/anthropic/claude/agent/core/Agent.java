...
public class Agent {
    ...
    public Agent(AgentConfig config, List<Tool> tools, AnthropicClient client) {
        this.config = config;
        this.client = client != null ? client : new AnthropicClient(config.getApiKey());
    ...
    public CompletableFuture<AgentResponse> chatAsync(String message) {
        if (config.isVerbose()) {
            System.out.println("[" + config.getName() + "] Received: " + message);
        }
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                while (true) {
                    ...
                    if (config.isVerbose()) {
                        System.out.println("[" + config.getName() + "] Response: " + response.getTextContent());
                        if (response.hasToolUses()) {
                            System.out.println("[" + config.getName() + "] Tool calls: " + response.getToolUses().size());
                        }
                    }
                    ...
                    if (response.hasToolUses()) {
                        List<ToolResult> toolResults = executeTools(response.getToolUses());
                        history.addToolResults(toolResults);
                        
                        if (config.isVerbose()) {
                            for (ToolResult result : toolResults) {
                                System.out.println("[" + config.getName() + "] Tool result: " + result.getContent());
                            }
                        }
                    } else {
    ...
    private Set<String> getBetaHeaders() {
        ...
        for (Tool tool : tools) {
            ...
            if ("computer".equals(tool.getName())) {
                ...
                if (model.contains("claude-4") || model.contains("claude-sonnet-3.7") || model.contains("claude-sonnet-4")) {
     ...
     */
    private List<ToolResult> executeTools(List<AgentResponse.ToolUseContentBlock> toolUses) {
        ...
        for (AgentResponse.ToolUseContentBlock toolUse : toolUses) {
            ...
            if (toolMap.containsKey(toolName)) {
                ...
                try {
                    ...
                    errorResult.setContent("Error executing tool: " + e.getMessage());
