...
public class ClaudeAgentCLI implements Callable<Integer> {
    ...
    static class ChatCommand implements Callable<Integer> {
        ...
        @Option(
            names = { "-p", "--prompt" },
            description = "Single prompt to send to the agent"
        )
        ...
        @Option(
            names = { "-t", "--tools" },
            split = ",",
            description = "Enable specific tools (comma-separated)"
        )
        ...
        public Integer call() throws Exception {
            ...
            // If config is null, create a default one
            if (config == null) {
        ...
        private Integer runSinglePrompt(
            AgentConfig config,
        ...
        private Agent createAgentWithTools(
            AgentConfig config,
            List<String> toolNames
        ) throws Exception {
            // Validate API key
            if (
                ...
                throw new IllegalStateException(
                    "No API key found. Please set the ANTHROPIC_API_KEY environment variable or provide --api-key"
                );
            }
            ...
            for (String toolName : toolNames) {
                try {
                    Map<String, Object> toolConfig = getToolConfig(
                        config,
                        toolName
                    );
        ...
        private Map<String, Object> getToolConfig(
            AgentConfig config,
            String toolName
        ) {
    ...
    static class InteractiveCommand implements Callable<Integer> {
        ...
        public Integer call() throws Exception {
            ...
            return chatCommand.runInteractiveSession(config, tools);
        }
    }
    ...
    static class ToolInfoCommand implements Callable<Integer> {
        ...
        public Integer call() throws Exception {
            ...
            try {
                ...
                System.out.println(
                    "Supports files: " + info.get("supports_files")
                );

                if (info.containsKey("tool_version")) {
                    System.out.println(
                        "Tool version: " + info.get("tool_version")
                    );
                }
    ...
    static class GenerateConfigCommand implements Callable<Integer> {
        ...
        public Integer call() throws Exception {
            ...
            if (outputFile != null) {
                ...
                System.out.println(
                    "Configuration saved to: " + outputFile.getAbsolutePath()
                );
            } else {
                ...
                System.out.println(
                    "  system_prompt: \"" + config.getSystemPrompt() + "\""
                );
                ...
                System.out.println(
                    "  max_tokens: " + config.getModelConfig().getMaxTokens()
                );
                System.out.println(
